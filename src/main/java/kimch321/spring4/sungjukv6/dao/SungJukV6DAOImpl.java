...
@Repository("sjdao")
public class SungJukV6DAOImpl implements SungJukV6DAO {
    ...
    public SungJukVO selectOneSungJuk(int sjno) {
        ...
        SungJukVO sj = null;
        try{
            sj = jdbcTemplate.queryForObject(selectOneSQL, mapper, param);
        } catch (Exception ex) {
            logger.error("selectOneSungJuk 에러!");
            ex.printStackTrace();
        }
    ...
    private class SungJukOneMapper implements RowMapper<SungJukVO> {
        @Override
        public SungJukVO mapRow(ResultSet rs, int num) {
            SungJukVO sj = null;
            try{
                ...
                logger.error("SungJukOneMapper 에러!");
                ex.printStackTrace();
            }
